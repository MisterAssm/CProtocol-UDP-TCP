...
int main(int argc, char *argv[]) {
  ...
  printColoredParts(
      {{"Saisir le contenu du fichier à envoyer au serveur :", BLUE}});

  std::cin.getline(buffer,
                   sizeof(buffer)); // Lecture avec gestion retour à la ligne
  buffer_length = strlen(buffer);   // Calculer la longueur de la chaîne
  ...
  if (sendto(sockfd, buffer, buffer_length, 0,
             (struct sockaddr *)&server_adress, sizeof(server_adress)) == -1) {
